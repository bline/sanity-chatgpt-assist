...
export const ChatHistoryList: React.FC<object> = () => {
    const [editingMessageId, setEditingMessageId] = useState<string | null>(null)
    const [editedContent, setEditedContent] = useState<string>('')
    const client = useClient({apiVersion: '2021-06-07'})
    const {chatHistory, setChatHistory} = usePromptDataContext()
    ...
    const handleDelete = async (messageId: string) => {
        if (chatHistory) {
            const updatedMessages = chatHistory.messages.filter(
                (message) => message._key !== messageId,
            )
    ...
    return (
        ...
        >
            {chatHistory?.messages?.map((message) => (
                <ChatBox colors={['#f9f9f9', '#333']} key={message._key}>
                        ...
                        <>
                            ...
                            <Flex style={{position: 'absolute', top: '-15px', right: 3}} gap={2}>
                                <Button
                                    ...
                                    mode="bleed"
                                    size={10}
                                    onClick={() => handleEdit(message._key, message.content)}
                                />
                                <Button
                                    icon={TrashIcon}
                                    mode="bleed"
                                    tone="critical"
                                    onClick={() => handleDelete(message._key)}
                                />
                            </Flex>
                        </>
                    )}
