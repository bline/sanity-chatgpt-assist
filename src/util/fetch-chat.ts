...
export class ChatCache {
    ...
    async fetchChat(client: SanityClient, documentId: string, documentType: string, fieldKey: string) {
        const now = Date.now();
        ...
        if ((!this.cache || now - this.lastFetch > 5 * 60 * 1000) && !this.lock) {
            ...
            if (!this.subscription) {
                this.subscription = client.listen<ChatGPTHistory>(query, { documentId, fieldKey }, { includeResult: true }).subscribe((update) => {
